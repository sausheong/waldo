...
func search(model string, query string) error {
	...
	prompt := `You are a helpful AI assistant who can answer questions from users. The 
follow search results has come back from a search engine given the query that came from 
a user. Respond to the original query using the search results. Add additional information 
if necessary. Assume the person you are explaining to doesn't know anything about the answer.	`
	ctx := `{
	"query" : "` + query + `",
	"search_result" : "` + data + `"
}`
...
func ddg(query string) (string, error) {
	...
	if response.StatusCode != http.StatusOK {
		return "", fmt.Errorf("status is %d", response.StatusCode)
	}
	...
	for i := range sel.Nodes {
		...
		results = append(results, SearchResult{title, info})
	}
	formattedResults := ""
